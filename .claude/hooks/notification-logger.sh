...
# Fleet Notification Logger — SubagentStop hook for crew agents
# Extracts FLEET_NOTIFICATION blocks from crew agent output and persists
# them as changelog entries in .fleet/changes/.
#
# Input: JSON on stdin with last_assistant_message field (SubagentStop payload)
# Output: Confirmation message if notifications found, silent otherwise

set -euo pipefail

# ── Dependencies ──────────────────────────────────────────────────────────────
command -v jq >/dev/null 2>&1 || { echo '{"error":"notification-logger: jq not found"}' >&2; exit 0; }

debug() { [[ -n "${FLEET_DEBUG:-}" ]] && echo "[notification-logger] $*" >&2 || true; }
...
if [[ -z "$AGENT_OUTPUT" ]]; then
  debug "empty output — nothing to log"
  exit 0
fi

# ── Check for FLEET_NOTIFICATION blocks ───────────────────────────────────────
if ! printf '%s\n' "$AGENT_OUTPUT" | grep -q '<!-- FLEET_NOTIFICATION'; then
...
BLOCKS=$(printf '%s\n' "$AGENT_OUTPUT" | sed -n '/<!-- FLEET_NOTIFICATION/,/-->/p')
...
printf '%s\n' "$BLOCKS" | while IFS= read -r line; do
  ...
  if printf '%s' "$line" | grep -q '^-->'; then
    ...
    # Idempotent write — skip if changelog entry with same crew+date+slug
    # already exists. This replaces the old blanket stop_hook_active guard:
    # - First stop: notifications logged (file doesn't exist yet)
    # - Second stop (after officer review): same notifications skipped (file
    #   exists), but genuinely NEW notifications get logged (different slug)
    CHANGELOG_FILE=".fleet/changes/${TODAY}-${CREW}-${SLUG}.md"
    if [[ -f "$CHANGELOG_FILE" ]]; then
      debug "changelog entry already exists: $CHANGELOG_FILE — skipping duplicate"
      continue
    fi
...
ENTRY_EOF
    ...
    # Output confirmation
    echo "Fleet: logged ${SEVERITY} notification from ${CREW} affecting ${PARTNERS}."

    if [[ "$SEVERITY" == "breaking" ]] || [[ "$SEVERITY" == "major" ]]; then
      debug "breaking/major notification from ${CREW} affecting ${PARTNERS} — logged to ${CHANGELOG_FILE}"
    fi
