            ...
            while i < n {
                ...
                if EXTERN_C_FN.is_match(trimmed) {
                    ...
                    if let (Some(start), Some(end)) = (fn_body_start, fn_body_end) {
                        ...
                        for (body_idx, body_line) in lines[(start + 1)..end].iter().enumerate().map(|(rel, l)| (start + 1 + rel, *l)) {
                            let body_trimmed = body_line.trim();

                            if body_trimmed.is_empty()
                                || is_comment(body_trimmed, Language::Rust)
                            {
                                continue;
                            }
...
"#
        ...
        let ctx = make_ctx(files);
        let findings = FfiPanicDetector.analyze(&ctx).await.unwrap();
        assert!(findings.is_empty(), "regular fn with unwrap should not flag");
    }
...
"#
        ...
        let ctx = make_ctx(files);
        let findings = FfiPanicDetector.analyze(&ctx).await.unwrap();
        assert!(findings.is_empty(), "Result-handled extern fn should not flag");
    }
