...
static LOCK_CALL: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\.lock\(\)").expect("lock regex must compile")
});
...
"#
        ...
        let ctx = make_ctx(files);
        let findings = MutexAcrossAwaitDetector.analyze(&ctx).await.unwrap();
        assert!(findings.is_empty(), "guard dropped before await should not flag");
    }
