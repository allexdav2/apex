...
impl FlakyReport {
    pub fn is_flaky(&self, id: &str) -> bool {
        self.flakiness_rate(id) > 0.0
    }
    pub fn flakiness_rate(&self, id: &str) -> f64 {
        *self.flakiness.get(id).unwrap_or(&0.0)
    }
...
mod tests {
    ...
    fn flakiness_rate_computed_correctly() {
        let mut det = FlakyDetector::new(4);
        for _ in 0..2 {
            det.record_run("t", true);
        }
        for _ in 0..2 {
            det.record_run("t", false);
        }
