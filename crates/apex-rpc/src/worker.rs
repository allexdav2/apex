...
mod tests {
    ...
    async fn setup_worker_uds() -> (
        ...
        let (service, _handle) =
            CoordinatorServer::start_uds_with_service(&sock_path, oracle.clone())
                .await
                .expect("failed to start UDS coordinator");
    ...
    async fn test_worker_id_is_uuid() {
        let (worker, _service, _oracle, _tmp) = setup_worker_uds().await;
    ...
    async fn test_register_succeeds() {
        let (mut worker, _service, _oracle, _tmp) = setup_worker_uds().await;
    ...
    async fn test_enqueue_and_get_seeds() {
        let (mut worker, service, _oracle, _tmp) = setup_worker_uds().await;
    ...
    async fn test_submit_results_updates_coverage() {
        let (mut worker, _service, oracle, _tmp) = setup_worker_uds().await;
    ...
    async fn test_pull_once_with_callback() {
        let (mut worker, service, oracle, _tmp) = setup_worker_uds().await;
    ...
    #[cfg(unix)]
    async fn setup_rejecting_worker_uds() -> (WorkerClient, tempfile::TempDir) {
        ...
        let uds = tokio::net::UnixListener::bind(&sock_path)
            .expect("failed to bind UDS for rejecting coordinator");
        ...
        let worker = WorkerClient::connect_uds(&sock_path, "python".into())
            .await
            .expect("failed to connect to rejecting coordinator over UDS");
    ...
    async fn test_register_rejected_returns_permission_denied() {
        let (mut worker, _tmp) = setup_rejecting_worker_uds().await;
    ...
    #[cfg(unix)]
    async fn setup_error_worker_uds() -> (WorkerClient, tempfile::TempDir) {
        use crate::proto::apex_coordinator_server::ApexCoordinatorServer;
        ...
        let uds = tokio::net::UnixListener::bind(&sock_path)
            .expect("failed to bind UDS for error coordinator");
        let incoming = tokio_stream::wrappers::UnixListenerStream::new(uds);
        ...
        let worker = WorkerClient::connect_uds(&sock_path, "rust".into())
            .await
            .expect("failed to connect to error coordinator over UDS");
    ...
    async fn test_error_register_propagates_status() {
        let (mut worker, _tmp) = setup_error_worker_uds().await;
    ...
    #[cfg(unix)]
    async fn setup_submit_error_worker_uds() -> (WorkerClient, tempfile::TempDir) {
        use crate::proto::apex_coordinator_server::ApexCoordinatorServer;
        ...
        let uds = tokio::net::UnixListener::bind(&sock_path)
            .expect("failed to bind UDS for submit-error coordinator");
        let incoming = tokio_stream::wrappers::UnixListenerStream::new(uds);
        ...
        let worker = WorkerClient::connect_uds(&sock_path, "python".into())
            .await
            .expect("failed to connect to submit-error coordinator over UDS");
    ...
    async fn test_pull_once_submit_error_propagated() {
        let (mut worker, _tmp) = setup_submit_error_worker_uds().await;
