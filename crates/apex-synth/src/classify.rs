...
impl GapClassifier {
    pub fn classify_source(snippet: &str) -> GapKind {
        if snippet.contains("except") || snippet.contains("catch") {
            return GapKind::ExceptionHandler;
        }
        if snippet.contains("None") || snippet.contains("null") || snippet.contains("nil") {
            return GapKind::NullCheck;
        }
        if snippet.contains('>')
            || snippet.contains('<')
            || snippet.contains(">=")
            || snippet.contains("<=")
        {
            return GapKind::BoundaryCondition;
        }
        GapKind::General
    }
}
